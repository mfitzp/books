...
# tag::calculate_delta[]
def calculate_delta(ball_y, ball_velocity_y, player_y):
    # Implement edge hits.
    if ball_velocity_y == 1 and ball_y == player_y - 1:
        return -2
    if ball_velocity_y == -1 and ball_y == player_y + BAT_SIZE:
        return 2

    # Normal slices/rebounds.
    if ball_y == player_y:
        return -1
    return 1
# end::calculate_delta[]
...
class Game:
    ...
    def computer_move(self):
        # Determine position of the ball and move towards it.
        ...
        if self.player_2_y < MIN_COORD:
            self.player_2_y = MIN_COORD

        if self.player_2_y + BAT_EXTRA > MAX_COORD:
            self.player_2_y = MAX_COORD - BAT_EXTRA
    ...
    def check_bats(self):
        ...
        if (
            ...
            veloc_delta = calculate_delta(self.ball_y, self.ball_velocity_y, self.player_1_y)
        ...
        if (
            ...
            veloc_delta = calculate_delta(self.ball_y, self.ball_velocity_y, self.player_2_y)
            self.ball_velocity_y = constrain_speed(self.ball_velocity_y + veloc_delta)
    # end::check_bats[]
